...
NOTE: AnyVision Media is NOT registered for VAT. Invoices generated by
PF-02 intentionally omit the "Tax Invoice" heading, VAT number, and VAT
breakdown — SARS prohibits non-VAT-registered vendors from issuing these.
...
AMOUNT_ZAR = 2500.00  # flat price — AVM is not VAT registered

# --- Business details for invoice ---
# NOTE: AnyVision Media is NOT registered for VAT. Invoices must not say
# "Tax Invoice" or show a VAT breakdown — SARS prohibits this for
# non-VAT-registered vendors. Keep AVM_VAT_NUMBER empty.
...
""".strip()
    ...
    # Merge ITN body with amount check. AVM is NOT VAT-registered,
    # so the full amount goes to Amount Excl VAT and VAT Amount stays 0.
    # The two Airtable columns are kept for schema consistency but the
    # invoice template below never shows a VAT line.
...
return { json: {
    ...
    amount_excl_vat: amountGross,   // AVM not VAT registered — full amount
    vat_amount: 0,
...
""".strip()
    ...
    # Compute invoice number from counter (handles HTTP Airtable response shape)
    invoice_num_js = _js_string(
        """
const itn = $('Check Amount + VAT').first().json;
const counterResp = $input.first().json;
const counterRec = counterResp.records && counterResp.records[0];

if (!counterRec) {
    throw new Error('CounterNotFound: invoice_sequence counter record not found in PF_Counters');
}

const currentValue = parseInt(counterRec.fields['Value'] || '0', 10) || 0;
...
// Look up the paid booking record ID (from Lookup Booking HTTP response)
const lookup = $('Lookup Booking').first().json;
const bookingRec = lookup.records && lookup.records[0];
if (!bookingRec) {
    throw new Error('BookingNotFound: no matching booking for m_payment_id=' + itn.m_payment_id);
}

return { json: {
    ...itn,
    counter_record_id: counterRec.id,
    counter_new_value: newValue,
    invoice_number: invoiceNumber,
    booking_record_id: bookingRec.id
}};
""".strip()
    )

    # Generate invoice HTML (not a "Tax Invoice" — AVM not VAT-registered)
...
<html><head><meta charset="UTF-8"><title>Invoice ${{input.invoice_number}}</title>
<style>
...
.vat-note {{ font-size: 0.78rem; color: #6B7280; margin-top: 12px; text-align: right; font-style: italic; }}
...
<div class="invoice">
    ...
    <div style="display: flex; justify-content: space-between; margin-bottom: 32px; gap: 40px;">
        <div class="meta" style="flex: 1;">
            ...
            ${{AVM_ADDRESS}}<br>
            ${{AVM_EMAIL}}${{AVM_REG ? '<br>Reg No: ' + AVM_REG : ''}}
        </div>
        <div class="meta" style="flex: 1;">
    ...
    <table class="item-table">
        ...
        <tbody>
            <tr>
                ...
                <td class="right">1</td>
                <td class="right">${{fmt(input.amount_gross_computed)}}</td>
                <td class="right">${{fmt(input.amount_gross_computed)}}</td>
            </tr>
        </tbody>
    </table>
    ...
    <div class="totals">
        ...
        <div class="vat-note">${{AVM_NAME}} is not registered for VAT. No VAT is charged.</div>
    </div>
    ...
    <div class="footer">
        This is an electronic invoice issued by ${{AVM_NAME}}.
...
""".strip()
    ...
    nodes: list[dict[str, Any]] = [
        ...
        # 5a. Build Airtable lookup URL in a Code node (bypasses the native
        #     Airtable node's broken filterByFormula expression evaluation)
        {
            "parameters": {
                "jsCode": f"""
const m_payment_id = $('Verify PayFast Signature').first().json.m_payment_id || '';
const safeId = String(m_payment_id).replace(/'/g, "\\\\'");
const formula = `{{Booking ID}} = '${{safeId}}'`;
const url = 'https://api.airtable.com/v0/{AIRTABLE_BASE_ID}/{PF_BOOKINGS_TABLE_ID}'
    + '?filterByFormula=' + encodeURIComponent(formula)
    + '&maxRecords=1';
return {{ json: {{ booking_lookup_url: url, m_payment_id: m_payment_id }} }};
""".strip(),
            },
            "id": uid(),
            "name": "Build Lookup URL",
            "type": "n8n-nodes-base.code",
            "typeVersion": 2,
            "position": [1100, 300],
        },
        # 5b. Look up booking via HTTP Request
        {
            "parameters": {
                "method": "GET",
                "url": "={{ $json.booking_lookup_url }}",
        ...
        # 7. Get invoice counter record via HTTP (same bypass pattern)
        {
            "parameters": {
                "method": "GET",
                "url": (
                    f"https://api.airtable.com/v0/{AIRTABLE_BASE_ID}/{PF_COUNTERS_TABLE_ID}"
                    "?filterByFormula=%7BCounter+Name%7D+%3D+%27invoice_sequence%27&maxRecords=1"
                ),
                "authentication": "predefinedCredentialType",
                "nodeCredentialType": "airtableTokenApi",
                "options": {"timeout": 15000},
        ...
        # 10. Update booking to paid
        {
            "parameters": {
                ...
                "base": {"__rl": True, "value": AIRTABLE_BASE_ID, "mode": "id"},
                "table": {"__rl": True, "value": PF_BOOKINGS_TABLE_ID, "mode": "id"},
                "id": "={{ $('Compute Invoice Number').first().json.booking_record_id }}",
                "columns": {
                    "mappingMode": "defineBelow",
                    "value": {
                        "Status": "paid",
        ...
        # 12. Send customer email via Gmail
        {
            "parameters": {
                ...
                "subject": "=Your AI Automation Consulting session is confirmed - Invoice {{ $('Compute Invoice Number').first().json.invoice_number }}",
                "emailType": "html",
...
def build_pf02_connections() -> dict[str, Any]:
    ...
    return {
        ...
        "Check Postback": {"main": [[{"node": "Build Lookup URL", "type": "main", "index": 0}]]},
        "Build Lookup URL": {"main": [[{"node": "Lookup Booking", "type": "main", "index": 0}]]},
...
def main() -> None:
    ...
    print("NOTE: AVM is not VAT registered — invoices omit 'Tax Invoice' heading + VAT breakdown")
    print()
